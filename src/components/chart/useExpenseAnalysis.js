...
export const useExpenseAnalysis = (businessId) => {
  ...
  // fetchExpenseData를 useCallback으로 분리
  const fetchExpenseData = useCallback(async () => {
    ...
    try {
      ...
      const totalExpense = byCategory.reduce((sum, x) => sum + x.amount, 0);
  ...
  // useEffect로 businessId 변경 시 재로드
  useEffect(() => {
    if (businessId) {
      fetchExpenseData();
    }
  }, [fetchExpenseData, businessId]);

  return { data, loading, fetchExpenseData }; // 필요 시 수동 재로딩도 가능
};
